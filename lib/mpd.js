...
module.exports = function(config) {
	async function handleCommand(cmd, params, con) {
		let resp = '';
		switch (cmd) {
			...
			case 'playlistinfo':
				...
				break;
			case 'status':
			let status = await ctrl.getStatus();
			resp = 
...
state: ${status.playing ? 'play' : 'pause'}
song: 0
songid: 0
nextsong: 1
nexsongid: 1
time: ${Math.round(status.time)}
elapsed: ${status.time}
...
`;
		...
		case 'stop':
			...
			break;
		case 'previous':
			ctrl.veto(con.socket.address().address);
			break;
		case 'next':
...
title: ${ctrl.playlist.current.title}
file: ${ctrl.playlist.current.path}
time: ${await ctrl.player.getDuration()}
id: 0
`;
			break;
		case 'add':
			ctrl.soundbit.play(params[0]);
			break;
		default:
