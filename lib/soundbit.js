...
function list(dir, cb) {
	...
	if(!cb) {
		return; // no use in doing anything if nobdy cares
	}
	...
	fs.readdir(dir, (err, files) => {
		...
		function addData(f, d) {			
			var n = path.parse(f).name;
			data[n] = d;
			// check if we complete the scan every data addition
			if (Object.keys(data).length >= files.length) {
				cb(null, data);
			}
		}
		
		// call callback directly if we have no files
		if(files.length == 0) {
			cb(null, data);
		}
		for(let f of files) {
			...
			fs.stat(file, (err, stats) => {
				if(err) {
					return; // can't call cb here in risk of multiple callback calls
				}
				if(stats.isDirectory()) {
					list(file, (err, items) => {
						// lets ignore the error and add the item to the data
						addData(f, items);
					});
				} else {
					addData(f, file.substring(soundBitDir.length)); // save relative path to file name
				}
			});
		}
	});
